...
export function BehavioralLearningIntro({ report }: { report: RepoReport }) {
  const trustNote = resolveGitTrustNote(report);

  return (
    <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
      <h2 className="text-lg font-semibold">{BEHAVIORAL_LEARNING_FRAMING.title}</h2>
      <p className="text-sm leading-relaxed text-foreground">{BEHAVIORAL_LEARNING_FRAMING.lead}</p>
      <p className="text-sm leading-relaxed text-muted-foreground">
        {BEHAVIORAL_LEARNING_FRAMING.discussion}
      </p>
      <p className="text-sm leading-relaxed text-muted-foreground">
        {BEHAVIORAL_LEARNING_FRAMING.teamVsAuthor}
      </p>
